...
if(in_array($fileType, $allowTypes)){ 
    ...
    $getImage = $newBD->query("SELECT photo FROM image WHERE user=$id"); 
    if ($getImage) {
        $newBD->query("DELETE FROM image WHERE user=$id");
    }
    $insert = $newBD->query("INSERT into image (photo,user) VALUES ('$imgContent',$id)"); 
...
<body>
  ...
  <div class="container my-5">
    ...
    <form action="paramétrage.php" class="d-flex justify-content-center border p-2 needs-validation bg-light shadow" novalidate  method="post" enctype="multipart/form-data">
                ...
                <input type="file"  id="inputGroupFile02" class="form-control w-100 m-3" name="photo" required>
                    <br>
